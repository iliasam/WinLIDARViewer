...
namespace LidarScanningTest1
{
    ...
    public partial class Form1 : Form
    {
        ...
        void ParseLdsFrame(List<MeasuredPointT> points)
        {
            ...
            {
                lblPacketCnt.Text = $"RX Frame CNT: {RXFrameCnt}";
            });
            ...
            for (int i = 0; i < pointsCnt; i++)
            {
                ...
                ScanPoints[i].WrongValue = (bool)(points[i].DistanceMM < 0) || (points[i].Intensity < 50);
        ...
        {
            int pointerAngleDeg = radarPlotComponent1.GetPointerAngle();

            //Find a point with a smallest angle difference
            int pointIndex = -1;
            double minDiffMM = 10000.0;
            for (int i = 0; i < ScanPoints.Length; i++)
            {
                double diffMM = Math.Abs(ScanPoints[i].RealAngleDeg - pointerAngleDeg);
                if (diffMM < minDiffMM)
                {
                    minDiffMM = diffMM;
                    pointIndex = i;
            ...
            int rawValue = ScanPoints[pointIndex].RawValue;
            double dist = ScanPoints[pointIndex].DistM;
